...
- **LLM 标签自动回填。** 档案入库时（及首次配置 LLM Key 时对全库的一次性回填），根据 `category:` 标签或已有分类名称，用 LLM 从标题中提取缺失的 `cosplayer:`/`artist:`/`circle:` 标签并写回，弥补分组建议算法依赖的结构化信号缺口；合集（anthology）不做单一创作者归属推断。
