...
除下方专门的 AI 插件创建向导外，其余 AI/LLM 能力均可配置、不配 DeepSeek Key 时优雅降级（纯本地嵌入模型或
直接跳过该功能），从不强制依赖：

- **智能推荐引擎。** 阅读到边界时展示推荐卡片（桌面 10 张/平板 6 张/手机 4 张），本地 ONNX 嵌入模型粗筛 +
  DeepSeek 大语言模型精排；Tankoubon 阅读边界以其最后一卷为锚点计算推荐，并排除同单行本其余卷。
- **AI Tankoubon 智能编辑。** 一键分析成员标题中的卷号/话数信息，自动建议单行本名称、章节标题和正确阅读
  顺序；支持整体命名和逐卷章节名智能建议，多候选翻页预览，即时应用。
- **AI 智能创建 Tankoubon。** 分析尚未加入任何 Tankoubon 的散档案，自动建议可能属于同一系列的分组，也会
  建议将散档案加入已存在的 Tankoubon；勾选确认后一键创建/添加，纯本地嵌入模型，不依赖 LLM Key。配置 LLM
  Key 后可选在创建时一并自动应用 AI 重命名与章节排序；不满意的建议可标记"不再提醒"。
- **LLM 标签自动回填。** 用 LLM 从标题中识别并补全缺失的作者/coser 标签，提升分组建议等功能的准确度。

#### AI 插件创建向导

- **根据自然语言描述生成登录/元数据/下载插件。** 输入目标域名后系统先查询该域名已有哪些类型的插件覆盖，
  用户只需为缺失的类型（最多三种）填写页面特征描述、测试链接（元数据/下载）或测试凭据（登录），AI（通过
  DeepSeek 工具调用循环）即可生成对应的 `.ts` 插件草稿——系统负责真实执行 AI 请求的页面抓取/试运行等
  一切网络访问，语义判断始终由 AI 完成，从不反过来。分步式向导界面每次只展示当前步骤，已覆盖的类型可选择
  "生成覆盖版本"或直接"编辑已有 AI 插件"（两者互不影响，可同时对不同类型分别操作）。
- **草稿先试运行，再确认保存。** 生成的草稿复用现有 Deno 沙箱的两阶段权限模型，针对用户提供的每个测试
  链接（或登录凭据）发起真实调用；试运行阶段绝不落地为正式插件，只有显式"确认保存"才会真正安装。手动
  编辑代码或让 AI 根据报错自动修复（最多连续 3 次，历史记录永不清空）都会重新触发试运行。
  测试凭据永远不会发送给 AI（仅登录调用本地执行、结果做脱敏后再给 AI 判断）。
- **试运行失败时的登录关联建议。** 若元数据/下载插件试运行失败，AI 会判断是否可能与登录有关，并引导
  用户就地补充生成一个登录插件（或直接关联已有的），成功后自动重新生成并关联，无需重启向导流程。
- **AI 生成的插件带有可见标记，且随时可导出。** 已安装插件列表中会用图标区分 AI 生成的插件，任意插件
  （无论是否由向导生成）都可以一键导出为 `.zip`；未挂载宿主机插件目录的部署形态下，保存的插件只存在于
  容器可写层中，保存成功后会提示用户导出备份。
- **插件新增 `domain_match` 字段区分"域名归属判断"与"真实触发匹配"。** 原本两者共用 `url_pattern`
  正则，导致向导的域名查找、上传页"获取元数据"按钮的可用性判断在插件路径写得较窄时出现误判；`domain_match`
  是一组纯域名字符串，专供归属判断使用，`url_pattern` 继续承担真实下载/元数据触发时的精确匹配，未声明时
  自动回退为把 `url_pattern` 当宽松域名正则，内置插件行为不受影响。
- **AI 插件保存操作的成功/失败均计入活动记录**，独立于普通手动上传插件的记录类型，可在 `/activity`
  按操作类型筛选查看。
