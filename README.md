...
- **消除了下载完成时的重复编目竞态。** 下载路径把文件 rename 进归档目录后自行编目，而基于 `notify` 的
  文件系统监视器几乎同时独立检测到同一次 rename、再编目一次——两条路径原本互不知晓，曾导致真实观测到的
  数据损坏（一个 331MB 的归档在被编目三次、两次相互竞争的重新键入之后，`pagecount` 永久停留在 0）。
  修复方式是把按文件名的互斥锁从下载路径专用（`lanrurugi-api::state`）下沉为跨 crate 共享的
  `lanrurugi_core::filename_lock::FilenameLocks`，文件系统监视器的消费循环现在会在同一把锁上排队等待，
  而不是各自独立编目。
- **`pagecount`/`arcsize` 元数据在检测到损坏时会自动自愈，而不是需要手动修复。** 每次服务启动、以及每次
  运行 `rebuild-index` 时，都会自动扫描 `pagecount` 为 0 但磁盘文件确实存在的归档，重新计算并写回正确的
  值；无法读取的归档（容器本身损坏或格式不受支持）会被记录一个失败时间戳，防止后续每一次扫描都重新尝试
  同一个永久性损坏的文件——只有重新编目该归档 ID（例如删除后重新下载覆盖）才会清除这个标记。结果会作为
  逐归档明细写入相应后台任务的 `/jobs` 结果中，而不是静默无感知地运行。
- **压缩包内单页图片损坏时提供故障隔离，而不是让整本归档的阅读体验受损。** 页面缩略图生成
  （`POST /archives/{id}/files/thumbnails`）在解码某一页失败时，会把该页记录为已知损坏；阅读器请求该页
  字节时会直接返回一张占位图，而不是重复尝试解码一张已知无法解码的图片、或是把损坏的原始字节透传给浏览器
  自行处理裂图。
...
- **服务重启不再让进行中的下载永久卡死。** 下载进度追踪（`JobRegistry`）纯粹存在于进程内存中，而队列项
  本身的状态持久化在 Redis 里——重启前处于"下载中"的队列项，其进度记录会随进程一起消失，但持久化的状态
  却原样保留，导致前端永远显示"正在开始…"且没有任何重试入口。服务启动时现在会扫描队列，把任何这样的悬空
  条目标记为失败并附带可辨识的原因，使其重新落入既有的"重试"操作，而不是静默卡死。
...
- **搜索语法新增空格作为条件分隔符，与逗号等效。** 旧版的 `compute_search_filter` 只把逗号当作真正的
  token 分隔符——像 `female:huge breasts female:milf` 这样用户自然会打出的、纯空格分隔的两个条件，会被
  静默拼成一个谁都匹配不到的长字符串（已通过真实 API 调用核实：单独搜索每一半各能找到 1 条结果，两者用
  空格拼在一起却是 0 条）。改为参考 e-hentai 真实搜索框的语法：空格和逗号一样成为 AND 分隔符，多词的标签
  值（如 `female:huge breasts` 本身就是一个词组，不是两个标签）需要用双引号整体包起来才能免于被拆开——
  凡是程序自动生成搜索串的地方（点击标签跳转、搜索框/分类谓词的自动补全插入）都已自动处理这层加引号逻辑，
  用户自己在搜索框里手动键入的裸多词查询若不加引号，才会按新语义被拆成多个独立关键字（与 e-hentai 行为
  一致，非静默的旧行为回归）。
- **Library 网格卡片新增"标记为已读/标记为未读"右键菜单项**，直接将阅读进度设为末页或 0——旧版没有任何
  脱离实际翻页动作、单独手动调整已读状态的方式。
- **服务端注入用户当前主题，消除首屏"默认主题闪烁"。** 后端在返回页面外壳时，直接把从 Redis 读到的真实
  主题写入一个 HTML 属性（`crates/lanrurugi-server/src/app.rs::serve_index`），页面内联的同步脚本据此在
  浏览器绘制第一帧之前就应用正确样式，慢网络下也不会先闪一下默认主题再切换成真实主题；本地开发模式下同一
  套注入通过 Vite 插件（`vite.config.ts::injectServerTheme`）复现，而不是仅在生产构建下生效。旧版没有这类
  机制——它的主题切换本身就是整页刷新，不存在这个问题，但也意味着它不需要、也没有解决过"单页应用如何避免
  首屏闪烁"这个问题。
- **分类下拉框旁新增"快速新建分类"入口**（阅读页概览、Upload 页、分类管理页三处共用同一个弹窗），无需先
  跳转到独立的分类管理页面。弹窗用静态/动态 tab 切换（而非分离的两个按钮），动态模式下的搜索谓词输入框
  带真实的 `namespace:tag` 自动补全（复用 Library 搜索栏同一套按片段匹配的建议逻辑），新建分类可以一步
  到位填好真正的谓词，而不必像旧版 `Category.addNewCategory` 那样先写死一个占位谓词、事后再去管理页手动
  改。旧版这三处的"添加到:"下拉框旁只有一个把*当前档案*加入*已选中*分类的按钮，从未提供过新建分类的快捷
  方式。
- **图章/标记功能新增选框（selection-box）标注**，在原有的单点图章之外，可拖拽画出一个矩形选区并附带
  emoji/Font Awesome 图标、8 向图标锚点、颜色/填充样式（纯色/斜线/马赛克/模糊）/边角样式（直角/圆角）/
  显示方式（悬停时/始终显示）；选中后可通过 8 个手柄拖拽缩放（按住 Shift 锁定当前像素宽高比）、方向键
  以 1px 步进微调位置、Ctrl+拖拽复制出一份独立新选框（原选框本身在整个拖拽过程中保持原位不动）。锚点/
  颜色/填充/边角/显示方式都会记住上一次*确认*绘制时的选择，下次新画选框时直接沿用；两个取色器都带悬浮
  提示，展示最近 6 个曾经确认使用过的颜色（各自独立记忆）。emoji 选择器的 9 个分类标签与全部约 1900 个
  emoji 自身名称在中文界面下使用 Unicode 官方 CLDR 简称数据本地化（而非机器翻译），其余语言暂沿用英文
  名称。旧版没有选框标注这一形式，只有单点图章。
- **Tankoubon（单行本）现在可以像一本连续的书一样阅读**，而不仅仅是一个用于批量打标签/管理的成员列表：
  阅读器把所有成员档案的页面按库内真实存储顺序拼接成一份连续页面流，翻页跨越成员档案边界时无缝衔接；
  阅读进度、书签、图章/选框标注、目录（ToC）增删、按页设置封面、清理缩略图缓存等原本"按档案"的功能全部
  正确解析到"当前全局页码所在的具体成员档案+本地页码"再操作，而不是整体失效或误操作到错误的档案。
  单行本自身的封面缩略图会在其依赖的来源档案（记录为具体的来源档案 ID + 页码，而非仅存一个可能过期的
  截图）不再是成员时自动回退到当前第一本档案的封面，若单行本已被清空则自动清除封面、前端显示占位图，
  而不是继续展示一张过期的、已不属于该单行本的缩略图。空单行本（所有成员都已被移出）不再从库首页
  的默认视图中消失变成不可发现的孤立数据——继续以 0 页显示，阅读页提供"编辑单行本"/"删除单行本"/
  "返回库"的直接入口。旧版对空单行本没有专门处理，也完全没有把 Tankoubon 当作连续读物阅读的能力。
...
**阅读到最后一页/第一页时，边界面板不再是空白——而是 10 张按相关性排列的推荐卡片。不是"你可能也喜欢"
这种模糊说辞，而是一个真实的两级推荐流水线：本地的 ONNX embedding 模型先做粗筛，DeepSeek 的大语言模型
再精排。旧版完全没有推荐功能。**

- **两级流水线：本地 embedding 粗筛 → LLM 精排。** 第一步：本地的 ONNX 嵌入模型（`multilingual-e5-small`，
  启动时自动下载）把全库每个归档的标题+标签编码为向量，用余弦相似度筛出前 100 个候选——这一步纯本地、零延迟、
  零网络请求。第二步：把这 100 个候选（标题+标签+页数）和当前阅读的漫画信息一起发给 DeepSeek 的 chat API，
  由 LLM 执行最终的「下一卷必须排第一」排序——这是嵌入模型永远做不到的事情（同一系列各卷之间的嵌入相似度差异
  几乎为零，无法区分卷 10 和卷 11）。

- **"下一卷第一"是硬规则，不是概率近似。** 系统提示词中明确写入了优先级规则：1) 同一系列的下一卷（当前卷号 +1）
  无条件排第一；2) 同系列其余各卷按卷号顺序紧随其后；3) 其余候选按相关度降序。这是嵌入模型完全无法做到的事情——
  它能判断"这两本书标题很相似"，但无法理解"卷 10 之后应该是卷 11 而不是卷 9 或卷 12"。LLM 能理解这种语义，
  而本地嵌入模型负责把全库范围缩小到 LLM 能处理的 100 个候选。

- **LLM 延迟被预取隐藏在翻页动作背后。** 推荐请求在翻到倒数第 3 页时就已经发出（TanStack Query prefetch），
  所以当你真正到达边界时，推荐卡片已经在缓存里了——秒开，无等待。如果预取没赶上（快速连翻最后几页），会先显示
  MUI 风格的呼吸骨架屏（10 张灰色占位卡），数据回来后自动填充。骨架屏而非空白——这是用户体验的基本礼仪。

- **API Key 安全设计：真实 key 永不到达前端。** 设置页的 DeepSeek API Key 字段只接收用户输入，保存后后端
  拥有 key；`GET /settings` 只返回 `llm_api_key_set: true/false`——前端知道"有没有配过 key"，但永远看不到
  key 本身。即使有人打开浏览器控制台敲 `$0.value`，也只能看到用户刚刚手动键入的字符，而不是已保存的真实值。
  启动时如果检测到 `DEEPSEEK_API_KEY` 环境变量（例如通过 compose/docker secret 注入），会自动写入 Redis，
  无需人工在设置页重复输入。

- **没有 API Key 也能正常使用。** 未配置 DeepSeek API Key 时，推荐引擎完全回退到纯嵌入排序——边界面板照常
  展示 10 张推荐卡片，只是缺少 LLM 的语义精排（同一系列的继续阅读可能不在第一位，但仍在候选池中）。
  每条候选都带着和 Library 卡片一样的 emoji 状态徽标（🆕 新入库 / 👑 已读完 / 📚 单行本），
  不是干巴巴的标题列表。

- **响应式卡片网格，不是固定布局。** 桌面端 5×2（10 张卡片），平板横屏 3×2（6 张），竖屏/手机 2×2（4 张），
  卡片尺寸用视口单位自适应缩放——没有一个固定写死的 px 值能在所有设备上看起来都对。遮罩背景是比标准遮罩更深
  的暗色（opacity 0.85），给推荐卡片制造一个沉浸式的 spotlight 效果；关闭按钮在右上角固定位置，不受滚动影响。
