 ...
 * "bookmark" mode).
 *
 * Only matters on a touch-only device (`useSupportsHover()` false) — a hover-capable device never
 * needs this at all, since `onMouseLeave` already closes a preview the instant the pointer leaves
 * it. On touch, there is no `mouseleave` equivalent: tapping a second card's cover would otherwise
 * leave the first card's preview open forever (nothing ever fires to close it), stacking multiple
 * `BookmarkHoverGrid` overlays. Keying by `archive.arcid` (not a per-instance id) means the same
 * archive appearing in two places at once (e.g. both the carousel and the page below it) shares
 * one open/closed state — a real but harmless edge case, not worth a more elaborate per-instance
 * identity scheme for. */
