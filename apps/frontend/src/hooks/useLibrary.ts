...
export function useLibrary() {
  ...
  useEffect(() => {
    ...
    // 007: a guest_visitor has no server-side progress to migrate *to* — every `PUT .../progress/*`
    // below is an admin-only write route (`route_policy.csv`) that would just 403, after already
    // showing a guest a "your progress has been migrated!" toast that describes something that
    // never actually happened server-side, plus firing a real network request per stray localStorage
    // key for no benefit. Skip the entire sweep for any unauthenticated session, eligible guest or
    // not — this migration only ever makes sense once a real admin session exists to migrate into.
    if (!loggedIn) return;
    if (
    ...
    void Promise.all(
      keys.map(async (id) => {
        ...
        const progressUrl = isTank
          ? `/api/tankoubons/${id}/progress/${localProgress}`
          : `/api/archives/${id}/progress/${localProgress}`;
        try {
