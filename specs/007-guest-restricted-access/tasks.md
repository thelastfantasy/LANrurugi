...
- [ ] T001 Add `visible_to_guest: bool` field to `Category` struct in
  `crates/lanrurugi-core/src/entities.rs` (entities.rs:118-135)
- [ ] T002 [P] Read/write `visible_to_guest` in `CategoryRepository::get`/`save` in
...
- [ ] T003 [P] Add `restrict_to_archive_ids: Option<HashSet<ArchiveId>>` field to `SearchParams` in
  ...
  research.md §4)
- [ ] T004 Add `AuthMethod::GuestVisitor` variant (no payload) to
  `crates/lanrurugi-api/src/auth_context.rs` (auth_context.rs:10-19; research.md §1)
- [ ] T005 Add `"guest_visitor"` match arm to `authz::subject_role` in
  `crates/lanrurugi-api/src/authz.rs` (authz.rs:49-62; research.md §2)
- [ ] T006 Add the `guest_visitor` Casbin policy block to
...
- [ ] T007 Add `guestmode: bool` to `BOOL_FIELDS` and remove `enablepass`/`nofunmode`/`devmode`
...
- [ ] T011 [P] [US1] Rewrite `test_app(enable_pass: bool)` helper in
...
- [ ] T012 [US1] Delete `nofunmode_forces_auth_even_when_enable_pass_is_off` test in
...
- [ ] T013 [P] [US1] Remove the `"nofun_mode"` field assertion from
...
- [ ] T014 [US1] Add a new integration test asserting an unauthenticated caller with no valid
...
- [ ] T015 [US1] Remove the `!cfg.enable_pass && !cfg.no_fun_mode` short-circuit branch from
  ...
  `enable_pass`)
- [ ] T016 [US1] Remove `enable_pass`/`no_fun_mode` from `LiveAuthConfig` and its `load()` in
...
- [ ] T017 [US1] Update `login.rs::status` — drop the `!auth.enable_pass ||` half of the
  `logged_in` expression (login.rs:259-280); depends on T016
- [ ] T018 [US1] Update `misc.rs::server_info` — hardcode `has_password: true`; remove `nofun_mode`
...
- [ ] T019 [P] [US1] Remove `enablepass`/`nofunmode` `CheckboxRow`s from
...
- [ ] T020 [P] [US1] Remove `devmode` `CheckboxRow` from
  `apps/frontend/src/pages/Settings/GlobalSection.tsx`
- [ ] T021 [US1] Drop `enablepass`/`nofunmode`/`devmode` from `useState`/`isDirty`/`handleSave` in
  `apps/frontend/src/pages/Settings/SettingsPage.tsx`; depends on T019, T020
- [ ] T022 [P] [US1] Remove `enablepass`/`nofunmode`/`devmode` from `Settings` interface, remove
  `nofun_mode` from `ServerInfo` interface (`has_password`/`debug_mode` stay) in
  `apps/frontend/src/api/types.ts`
- [ ] T023 [P] [US1] Remove `enablepass`/`nofunmode`/`devmode` entries from
  `SETTINGS_FIELD_SECTIONS` in `apps/frontend/src/pages/Activity/activityTarget.ts`
- [ ] T024 [P] [US1] Remove `enablePassword`/`nofunMode`/`debugMode` i18n keys from all 13
...
- [ ] T025 [P] [US2] Add a `guestmode`/category-visibility test matrix to
...
- [ ] T026 [P] [US2] Add a new integration test verifying an unauthenticated `guest_visitor`
...
- [ ] T027 [P] [US2] Add a `CategoryRepository::visible_to_guest` round-trip unit test (absent in
  Hash → `false`) alongside existing `pinned` test coverage in
  `crates/lanrurugi-storage/src/repository.rs`'s `mod tests` (repository.rs:850+)
- [ ] T028 [P] [US2] Add an `authz::subject_role` unit test for the new `guest_visitor` branch in
  `crates/lanrurugi-api/src/authz.rs`'s `mod tests` (authz.rs:231+)

### Implementation for User Story 2

- [ ] T029 [US2] In `procedure.rs::require_api_key`, before the final `401` (procedure.rs:130), add
...
- [ ] T030 [US2] Add `"visible_to_guest": 0|1` to `category_json()` in
...
- [ ] T031 [P] [US2] Add `guest_mode_enabled: boolean` to `LoginStatus` interface in
...
- [ ] T032 [US2] Add `guestmode` `CheckboxRow` to
  ...
  on T021, T031
- [ ] T033 [US2] Add a "visible to guest" checkbox next to the existing `pinned` checkbox in
...
- [ ] T034 [US2] Add a new `AllowGuest` guard component in `apps/frontend/src/RouteGuards.tsx`
...
- [ ] T035 [US2] In `apps/frontend/src/App.tsx` (App.tsx:57-74), move the Library (`/`) and Reader
...
- [ ] T036 [US2] In `apps/frontend/src/Layout.tsx` (Layout.tsx:32-45), make the nav-link logic
...
- [ ] T037 [P] [US3] Add a `SearchParams.restrict_to_archive_ids` filtering unit test (empty set
  ...
  T003
- [ ] T038 [P] [US3] Add an integration test: guest search/browse results and tag-filter results
...
- [ ] T039 [P] [US3] Add an integration test: a guest's direct request for an out-of-scope archive
...
- [ ] T040 [P] [US3] Add an integration test: guest `POST` bookmark, `PUT` progress, and `GET`
...
- [ ] T041 [P] [US3] Add an integration test: guest requests to `/api/plugins`, `/api/activity`,
  `/api/stats` all reject regardless of guest mode state, in
  `crates/lanrurugi-server/tests/settings_toggles.rs` (quickstart.md §9)
- [ ] T042 [P] [US3] Add an integration test: a config change (guest mode off, or last
...
- [ ] T043 [US3] In `search.rs::search_archives`/`search_archive_ids` (search.rs:184-217), when the
  ...
  `SearchParams.restrict_to_archive_ids`; depends on T003, T029, T030
- [ ] T044 [US3] In `archives.rs::get_archive_metadata`/`get_archive_deprecated`
  (archives.rs:312-329), when the caller is `AuthMethod::GuestVisitor`, verify the archive belongs
  to at least one `visible_to_guest: true` category (via `CategoryRepository::for_archive`) before
  proceeding; return the same `not_found()` response used for a genuinely nonexistent archive when
  it doesn't (research.md §6); depends on T029, T030
- [ ] T045 [US3] Apply the same guest-scope membership check as T044 to `get_page`
  (archives.rs:1973-2057), `get_files`, and `get_page_dimensions` in
  `crates/lanrurugi-api/src/archives.rs`; depends on T044
- [ ] T046 [US3] Confirm (via T040) that `/api/archives/{id}/download` (archives.rs:767-818)
  ...
  download gating is policy-level, not code-level
- [ ] T047 [P] [US3] In `apps/frontend/src/pages/Library/` and `apps/frontend/src/pages/Reader/`,
  hide bookmark/progress-save/download affordances when `useLoginStatus().data.logged_in` is
  `false` (both already consume `useLoginStatus()` elsewhere in the app per plan.md)
- [ ] T048 [P] [US3] Add a Playwright end-to-end test covering the guest journey — land on the
  ...
  convention; extends 003-ui-test-automation's Playwright layer)
- [ ] T049 [P] [US3] Add a Vitest unit test for `RouteGuards.tsx`'s new `AllowGuest` branch
...
- [ ] T050 [P] Remove the entire `anonymous` Casbin subject block from
  ...
  access after this feature)
- [ ] T051 [P] Add a short explanatory comment block above the new `guest_visitor` policy section
  ...
  guest_visitor-specific)
- [ ] T052 Run `cargo test --workspace` (via `mise run test`, per this repo's own resource-capped
  script convention) and confirm all Phase 2-5 tests pass, plus no pre-existing test broke
