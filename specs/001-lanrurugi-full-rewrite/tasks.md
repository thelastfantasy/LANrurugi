...
the single `lanrurugi-server` binary, per plan.md/constitution Principle III), including the
benchmark harness at `crates/lanrurugi-bench/`; frontend under `apps/frontend/src/...`. See
...
- [X] T001 Create Cargo workspace `Cargo.toml` at repo root with members
...
- [X] T002 [P] Initialize frontend Vite + React 19 + TypeScript + Tailwind project in
...
- [X] T003 [P] Create `.mise.toml` at repo root pinning exact Rust/Node/Deno/`pnpm` versions,
      ...
      Constraints
- [X] T004 [P] Adapt `lefthook.yml` at repo root from `~/jellyfin-suite/lefthook.yml`, adding a
...
- [X] T005 [P] Adapt `frontend/eslint.config.mjs` from
      `~/jellyfin-suite/apps/frontend/eslint.config.mjs` (memory:
      `jellyfin-suite-tooling-reference`)
- [X] T006 [P] Write `Dockerfile` at repo root: Debian `bookworm-slim` base, multi-stage copy of a
      ...
      Constraints
- [X] T007 [P] Configure CI pipeline in `.github/workflows/ci.yml` running `cargo check`,
...
- [X] T008 Define core domain types (`Archive`, `Category`, `Grouping`, `ReadingProgress`,
      `Extension`, `Stamp`) in `crates/lanrurugi-core/src/entities.rs`, per `data-model.md`
- [X] T009 [P] Implement the legacy archive-ID algorithm (SHA-1 of first 512KB) in
      `crates/lanrurugi-storage/src/id.rs`
- [X] T010 [P] Implement the size-aware archive-ID algorithm (SHA-1 of first 512KB ++ u64
...
- [X] T011 Implement the Redis connection pool (`redis` + `deadpool-redis`) in
      `crates/lanrurugi-storage/src/redis.rs`
- [X] T012 Implement Redis read/write mappers for Archive/Category/Grouping/ReadingProgress/Stamp
      in `crates/lanrurugi-storage/src/repository.rs`, matching legacy field names/key shapes
      verified in `data-model.md`
- [X] T013 Implement a generic job-status tracking abstraction (store + polling) in
...
- [X] T014 Set up the Axum app skeleton and router assembly in
      `crates/lanrurugi-server/src/app.rs`
- [X] T015 Implement API-key authentication middleware in
      `crates/lanrurugi-server/src/middleware/auth.rs`, matching legacy auth semantics
      (`contracts/rest-api.md`)
- [X] T016 Set up the `clap` CLI with `serve` / `rebuild-index` / `bench` subcommands in
      `crates/lanrurugi-server/src/main.rs`
- [X] T017 Set up structured logging and error-handling infrastructure in
      `crates/lanrurugi-server/src/telemetry.rs`
- [X] T018 [P] Implement the rayon-thread-pool + `tokio::task::spawn_blocking` bridging helper in
...
- [X] T019 [US1] Implement the library-open/bootstrap routine (verifies Redis reachable, archive
      folder reachable, no destructive step) in `crates/lanrurugi-storage/src/bootstrap.rs`
- [X] T020 [P] [US1] Implement `GET /archives` listing endpoint in
      `crates/lanrurugi-api/src/archives.rs`, per `contracts/rest-api.md`
- [X] T021 [P] [US1] Implement `GET /archives/{id}/metadata` endpoint in
      `crates/lanrurugi-api/src/archives.rs`
- [X] T022 [P] [US1] Implement `GET /categories` and `GET /tankoubons` listing endpoints
      preserving grouping/volume order in `crates/lanrurugi-api/src/categories.rs` and
      `crates/lanrurugi-api/src/tankoubons.rs`
- [X] T023 [P] [US1] Implement `GET /archives/{id}/progress/{page}` (read) in
      `crates/lanrurugi-api/src/archives.rs`
- [X] T024 [P] [US1] Implement `GET /archives/{id}` (full detail) and `DELETE /archives/{id}` in
      `crates/lanrurugi-api/src/archives.rs`
- [X] T025 [P] [US1] Implement `GET /archives/untagged` in `crates/lanrurugi-api/src/archives.rs`
- [X] T026 [P] [US1] Implement `GET /archives/{id}/thumbnail` in
      `crates/lanrurugi-api/src/archives.rs`
- [X] T027 [US1] Implement `GET /archives/{id}/categories` and `GET /archives/{id}/tankoubons`
      (per-archive membership) in `crates/lanrurugi-api/src/archives.rs`
- [X] T028 [US1] Implement `GET /archives/{id}/toc` and `GET /archives/{id}/download` in
      `crates/lanrurugi-api/src/archives.rs`
- [X] T029 [P] [US1] Implement `GET /archives/{id}/isnew` in `crates/lanrurugi-api/src/archives.rs`
- [X] T030 [US1] Implement Stamp repository access plus `GET /archives/{id}/stamps`,
      ...
      `data-model.md`)
- [X] T031 [US1] Implement `POST /categories/bookmark_link`, `GET /categories/bookmark_link/{id}`,
      `GET/PUT/DELETE /categories/{id}`, `PUT/DELETE /categories/{id}/{archive}` in
      `crates/lanrurugi-api/src/categories.rs`
- [X] T032 [US1] Implement `GET/PUT/DELETE /tankoubons/{id}`, `GET /tankoubons/{id}/full`,
...
- [X] T033 [P] [US1] Implement `GET /info` in `crates/lanrurugi-api/src/misc.rs`
- [X] T034 [US1] Build the library-browse page (list archives/categories/tankoubons with existing
      metadata) in `frontend/src/pages/Library.tsx`
- [X] T035 [US1] Run `quickstart.md` §1 against a real/representative legacy Redis dataset and
      confirm SC-001
...
- [X] T036 [US2] Implement the `notify`-based file watcher with a 3–5s debounce window in
      `crates/lanrurugi-scanner/src/watcher.rs`, per `research.md` §6
- [X] T037 [US2] Implement the per-file ingestion timeout (~30s) in
      `crates/lanrurugi-scanner/src/pipeline.rs`
- [X] T038 [US2] Implement the mpsc-based ingestion pipeline in
      `crates/lanrurugi-scanner/src/pipeline.rs`
- [X] T039 [P] [US2] Implement rayon-parallel batch hashing for bulk scans (bridged via
      `spawn_blocking`) in `crates/lanrurugi-scanner/src/hashing.rs`, per constitution
      Principle III
- [X] T040 [US2] Implement the "wait until file is stable" partial-write check in
      `crates/lanrurugi-scanner/src/watcher.rs` (FR-006)
- [X] T041 [US2] Implement new-archive cataloguing (create an Archive record via
      `lanrurugi-storage`, size-aware ID by default) in `crates/lanrurugi-scanner/src/pipeline.rs`
- [X] T042 [US2] Implement archive-format handling — `zip` crate for ZIP; shell out to
      `unrar`/`7z` for RAR/7z — and page-count extraction in
      `crates/lanrurugi-scanner/src/archive_format.rs`, per `research.md` §3
- [X] T043 [US2] Implement thumbnail generation (`image` crate, rayon + `spawn_blocking`) in
      `crates/lanrurugi-scanner/src/thumbnail.rs`, per `research.md` §4
- [X] T044 [US2] Implement an optional background full-verification job that re-checks
...
- [X] T045 [P] [US2] Implement `POST /shinobu/rescan`, `GET /shinobu`, `POST /shinobu/stop`,
      `POST /shinobu/restart` endpoints in `crates/lanrurugi-api/src/shinobu.rs`
- [X] T046 [US2] Implement `POST /archives/upload` and the `/tempfolder` staging endpoint in
      `crates/lanrurugi-api/src/upload.rs`, reusing T041's cataloguing logic
- [X] T047 [P] [US2] Implement `POST /regen_thumbs` in `crates/lanrurugi-api/src/archives.rs`,
      reusing T043's thumbnail logic
- [X] T048 [US2] Run `quickstart.md` §2 and confirm SC-002/SC-003 — both archives sharing a
...
- [X] T049 [US3] Implement the search query grammar parser (namespace:tag, wildcards, boolean) in
      `crates/lanrurugi-search/src/grammar.rs`, per `research.md` §8
- [X] T050 [US3] Port the Redis-based search execution (title sorted set, tag-set filtering,
      search-results cache keyed by filter/sort/flags) in
      `crates/lanrurugi-search/src/engine.rs`
- [X] T051 [P] [US3] Implement `GET /search`, `GET /search/ids`, `GET /search/random`,
      `GET /search/cache` endpoints in `crates/lanrurugi-api/src/search.rs`, per
      `contracts/rest-api.md`
- [X] T052 [P] [US3] Implement `PUT /archives/{id}/metadata` (tag/title/summary update) in
      `crates/lanrurugi-api/src/archives.rs`
- [X] T053 [P] [US3] Implement `GET /archives/{id}/page`, `GET /archives/{id}/files`,
      `GET /archives/{id}/files/thumbnails` endpoints in `crates/lanrurugi-api/src/archives.rs`
- [X] T054 [US3] Implement `GET /opds`, `GET /opds/{id}`, `GET /opds/{id}/pse` endpoints in
      `crates/lanrurugi-api/src/opds.rs`
- [X] T055 [US3] Write a contract-replay test suite (records legacy request/response pairs and
...
- [X] T056 [US3] Run `quickstart.md` §3 against an existing third-party client and confirm SC-004
...
- [X] T057 [US4] Implement the Deno subprocess worker-pool manager in
      `crates/lanrurugi-plugin/src/pool.rs`, per `research.md` §7 and constitution Principle IV
- [X] T058 [US4] Implement the newline-delimited JSON request/response protocol client in
      `crates/lanrurugi-plugin/src/protocol.rs`, per `contracts/plugin-protocol.md`
- [X] T059 [US4] Implement permission-flag construction (`--allow-net=<hosts>` etc.) from a
      plugin's declared `plugin_info` in `crates/lanrurugi-plugin/src/permissions.rs`
- [X] T060 [US4] Write the Deno dispatcher script (dynamic `import()` of the requested plugin
      module) in `crates/lanrurugi-plugin/dispatcher/dispatcher.ts`
- [X] T061 [US4] Implement per-request timeout and failure isolation in
      `crates/lanrurugi-plugin/src/pool.rs` (FR-013)
- [X] T062 [P] [US4] Implement `GET /plugins/{type}`, `POST /plugins/use`, `POST /plugins/queue`
      endpoints in `crates/lanrurugi-api/src/plugins.rs`
- [X] T063 [US4] Write a sample metadata plugin exercising the protocol in
      `crates/lanrurugi-plugin/samples/sample-metadata-plugin.ts`
- [X] T064 [P] [US4] Implement `POST /download_url` (download-type plugin trigger) in
      `crates/lanrurugi-api/src/plugins.rs`
- [X] T065 [US4] Run `quickstart.md` §4 and confirm FR-012–FR-014 (enrichment, isolation,
...
- [X] T066 [US5] Implement the backup-JSON builder (categories/tankoubons/stamps/archives) in
      `crates/lanrurugi-backup/src/build.rs`, matching `data-model.md`'s Backup/Export document
      shape
- [X] T067 [US5] Implement the consistent-snapshot guarantee (no partial/corrupt output under
      concurrent writes) in `crates/lanrurugi-backup/src/build.rs` (FR-010)
- [X] T068 [US5] Implement restore-from-backup logic in `crates/lanrurugi-backup/src/restore.rs`
- [X] T069 [P] [US5] Implement `POST /database/backup`, `GET /database/backup/{jobid}`,
...
- [X] T070 [P] [US5] Implement `GET /database/stats`, `GET /database/isnew`,
      `POST /database/drop`, `POST /database/clean` in `crates/lanrurugi-api/src/database.rs`
- [X] T071 [US5] Implement the generic `GET /minion/{jobid}`, `GET /minion/{jobid}/detail`,
...
- [X] T072 [US5] Build backup/restore UI controls in `frontend/src/pages/Settings/Backup.tsx`
- [X] T073 [US5] Run `quickstart.md` §5 and confirm SC-009
...
- [X] T074 [US6] Implement the rebuild-index core logic (recompute IDs, re-key records, split
      previously-merged pairs per `data-model.md`'s "Rebuild/Reindex operation") in
      `crates/lanrurugi-storage/src/rebuild.rs`
- [X] T075 [US6] Implement reference updates (Category/Grouping/Stamp re-keying to the new
      Archive ID) in `crates/lanrurugi-storage/src/rebuild.rs` (FR-012)
- [X] T076 [P] [US6] Implement `POST /database/rebuild-index` and its job-status polling (reusing
      the T013/T071 job-tracking abstraction) in `crates/lanrurugi-api/src/database.rs`, per
      `contracts/rest-api.md`
- [X] T077 [US6] Wire the `lanrurugi rebuild-index` CLI subcommand in
      `crates/lanrurugi-server/src/cli/rebuild_index.rs`
- [X] T078 [US6] Run `quickstart.md` §6 and confirm SC-005
...
- [X] T079 [US7] Convert the 14 legacy `locales/template/*.po` files to i18next-shaped JSON in
      `frontend/src/i18n/locales/*.json`, per `research.md` §10
- [X] T080 [US7] Set up `react-i18next` with English fallback in `frontend/src/i18n/index.ts`
- [X] T081 [P] [US7] Build the language-selector UI control in
      `frontend/src/components/LanguageSelector.tsx`
- [X] T082 [US7] Wire existing UI strings through the i18n layer across `frontend/src/`
      (sweep pass — no untranslated literal strings left in components)
- [X] T083 [US7] Run `quickstart.md` §7 and confirm SC-010
...
- [X] T084 [US8] Implement the synthetic-library generator (~100,000 archives, configurable
      smaller scale) in `bench/synthetic-library/generate.rs`
- [X] T085 [P] [US8] Implement `criterion` microbenchmarks (hashing throughput, thumbnail
      decode/resize) in `bench/criterion/benches.rs`
- [X] T086 [US8] Implement the cross-system comparison harness (drives both the legacy LANraragi
      instance and the new binary against the same synthetic library) in
      `bench/compare/run.rs`
- [X] T087 [US8] Implement benchmark-report generation matching
      `contracts/benchmark-report.md` in `bench/compare/report.rs`
- [X] T088 [US8] Implement a search/browse responsiveness benchmark against the synthetic
...
- [X] T089 [P] [US8] Implement `POST /bench/run`, `GET /bench/{reportid}` endpoints in
      `crates/lanrurugi-api/src/bench.rs`
- [X] T090 [US8] Wire the `lanrurugi bench` CLI subcommand in
      `crates/lanrurugi-server/src/cli/bench.rs`
- [X] T091 [US8] Run `quickstart.md` §8 (including the single-core-host edge case) and confirm
...
- [X] T092 [P] Update `README.md`/`docs/` to reflect the shipped Phase 1 feature set
- [X] T093 Code cleanup and refactoring pass across `crates/`
- [X] T094 [P] Security hardening pass — plugin permission audit (`crates/lanrurugi-plugin/`),
      API-key handling review (`crates/lanrurugi-server/src/middleware/auth.rs`)
- [X] T095 Run the full `quickstart.md` end-to-end across all 8 Phase 1 stories on a clean
      checkout
- [X] T096 [P] Verify the Docker image (`Dockerfile` from T006) builds and runs
