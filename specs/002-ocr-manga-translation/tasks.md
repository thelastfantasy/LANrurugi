...
**Input**: Design documents from `/specs/002-ocr-manga-translation/`
...
new frontend code under `frontend/src/translation/...` and new components alongside Phase 1's
existing `frontend/src/components/`, `frontend/src/pages/`. See `plan.md` § Project Structure.
...
- [ ] T005 [P] Create the `frontend/src/translation/` directory skeleton with barrel exports in
      `frontend/src/translation/index.ts`
...
- [ ] T022 [P] Implement the `localStorage` wrapper and device-local precedence resolution
      (FR-003, research.md §8) in `frontend/src/translation/settings.ts`
- [ ] T023 [P] Implement the IndexedDB/Cache API wrapper for the client-composited cache
      (research.md §7) in `frontend/src/translation/cache.ts`
...
- [ ] T024 [US1] Implement the enable/disable toggle for on-page translation (FR-001) in
      `frontend/src/components/TranslationSettings.tsx`
...
- [ ] T027 [US1] Implement the target-language selection UI with browser-language fallback
      (FR-004) in `frontend/src/components/TranslationSettings.tsx`
...
- [ ] T037 [P] [US2] Implement a "reset font pattern" UI control in
      `frontend/src/components/VolumeFontPatternControl.tsx`
...
- [ ] T041 [P] [US3] Implement the non-obscuring loading indicator component in
      `frontend/src/translation/components/LoadingIndicator.tsx`
- [ ] T042 [US3] Implement client-side ready-detection polling that, upon seeing T040's
      ...
      previously unassigned) in `frontend/src/translation/readyPoller.ts`
...
- [ ] T045 [P] [US3] Implement the usage panel (page/archive/day/week breakdown, optional chart)
      in `frontend/src/translation/components/UsagePanel.tsx`
- [ ] T046 [US3] Implement abandonment of in-flight look-ahead requests on navigate-away
      (FR-015) in `frontend/src/translation/prefetchController.ts`
...
- [ ] T054 [US4] Confirm the device-local backend selection (T022) takes precedence over the
      server-stored default end-to-end in `frontend/src/translation/settings.ts`
...
- [ ] T057 [US5] Implement fallback-to-original-page display on any translation failure (FR-019)
      in `crates/lanrurugi-api/src/translation.rs` and `frontend/src/pages/Reader.tsx`
...
- [ ] T059 [US5] Implement a guided configuration prompt when translation is enabled with no
      backend configured (FR-021) in `frontend/src/components/TranslationSettings.tsx`
...
- [ ] T062 Code cleanup and refactoring pass across `crates/lanrurugi-ocr/`,
      `crates/lanrurugi-fontcache/`, `crates/lanrurugi-translate/`, `frontend/src/translation/`
