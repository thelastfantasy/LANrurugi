...
//   - this plugin declares 7 custom parameters, but the host currently passes only one generic 
...
export function pluginInfo() {
  return {
    ...
    // (~/LANraragi/lib/LANraragi/Plugin/Metadata/EHentai.pm): the gallery search/parse pages
    // (e-hentai.org, exhentai.org when the ExHentai option is enabled) and the metadata API
    // (api.e-hentai.org).
...
export async function execMetadata(hostArgs: Record<string, unknown>) {
  ...
  let lang = hostArgs.arg as string;
  let usethumbs = hostArgs.arg as string;
  let search_gid = hostArgs.arg as string;
  let enablepanda = hostArgs.arg as string;
  let jpntitle = hostArgs.arg as string;
  let additionaltags = hostArgs.arg as string;
  let expunged = hostArgs.arg as string;
  //# Plugin parameters
  ...
  if ((match = lrr_info["oneshot_param"].match(/.*\/g\/([0-9]*)\/([0-z]*)\/*.*/))) {
    gID = match[1];
    gToken = match[2];
