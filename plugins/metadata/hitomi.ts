// Converted from LANraragi/lib/LANraragi/Plugin/Metadata/Hitomi.pm via
// `lanrurugi-plugin-converter` (`cargo run -p lanrurugi-plugin-converter -- convert ...`),
// regenerated by `scripts/convert-plugins.sh` on 2026-07-09.
//
// Known limitations (from the converter's own warnings — review, don't blindly trust):
//   - =~ with /g in list context (/\{?(\d+)\}?/gm) — Perl returns every captured group across all 
//     matches; JS .match() with /g returns only full-match strings, not captures. Verify this call 
//     site by hand.
//   - =~ with /g in list context 
//     (/.*source:\s*hitomi\.la\/(?:manga|doujinshi|galleries|cg)\/.*-?([0-9]*)\.html.*/gi) — Perl 
//     returns every captured group across all matches; JS .match() with /g returns only full-match 
//     strings, not captures. Verify this call site by hand.
//   - =~ with /g in list context (/var.*galleryinfo.*= (.*)/gmi) — Perl returns every captured 
//     group across all matches; JS .match() with /g returns only full-match strings, not captures. 
//     Verify this call site by hand.
//   - external Perl module reference has no JS equivalent: Mojo::UserAgent
//

declare global {
  interface PerlTransaction {
    req: { headers: { header(name: string, value: string): void } };
  }
  interface PerlUserAgent {
    cookie_jar: { add(cookie: { name: string; value: string; domain: string; path: string }): void };
    max_redirects(n: number): PerlUserAgent;
    transactor: { name(value: string): void };
    on(event: "start", handler: (ua: PerlUserAgent, tx: PerlTransaction) => void): void;
    get(url: string): Promise<{ result: PerlHttpResult }>;
    post(url: string, kind: "form" | "json", data: Record<string, string> | Record<string, unknown>): Promise<{ result: PerlHttpResult }>;
    cookies: { name: string; value: string; domain: string; path: string }[];
  }
  interface PerlHttpResult {
    body: string;
    code: number;
    readonly dom: PerlDomNode;
    readonly json: unknown;
  }
  interface PerlLogger {
    debug(msg: string): void;
    info(msg: string): void;
    warn(msg: string): void;
    error(msg: string): void;
  }
  interface PerlDomNode {
    text: string;
    attr(name: string): string | undefined;
    parent: PerlDomNode | undefined;
    at(selector: string): PerlDomNode | undefined;
    find(selector: string): PerlDomNode[] & { each<T>(fn: (node: PerlDomNode, index: number) => T): T[] };
    toString(): string;
  }
  // deno-lint-ignore no-var
  var perlCompat: {
    reverse<T>(list: readonly T[]): T[];
    chomp(s: string): string;
    sprintf(format: string, ...args: unknown[]): string;
    userAgent(): PerlUserAgent;
    getLogger(name: string, category: string): PerlLogger;
    htmlUnescape(s: string): string;
    parseHtml(markup: string, xml?: boolean): PerlDomNode;
    sleep(seconds: number): Promise<void>;
    getVersion(): { version: string; homepage: string };
    refType(x: unknown): string;
    trim(s: string | null | undefined): string;
    fileparse(path: string, suffixPattern?: unknown): [string, string, string];
    redis_decode(s: string): string;
  };
}

export function pluginInfo() {
  return {
    namespace: "hitomiplugin",
    type: "metadata" as const,
    parameters: [
    ],
    // TODO(perl-convert): source used an HTTP client (Mojo::UserAgent/LWP/etc.) — 
    // fill in the actual host(s) this plugin needs so Deno's --allow-net grant 
    // stays as narrow as possible (constitution Principle IV).
    // Hosts verified against every URL literal in the original Perl source
    // (~/LANraragi/lib/LANraragi/Plugin/Metadata/Hitomi.pm): the gallery page (hitomi.la) and the
    // gallery-info JS endpoint it scrapes (ltn.gold-usergeneratedcontent.net).
    declared_permissions: {
      net: ["hitomi.la", "ltn.gold-usergeneratedcontent.net"],
      read: false,
      write: false,
    },
    name: "Hitomi",
    author: "doublewelp (ported)",
    description: "Searches Hitomi.la for tags matching your archive.\n          <br>Supports reading the ID from files formatted as \"{Id} Title\" (curly brackets optional)\n\t\t  <br><i class='fa fa-exclamation-circle'></i> This plugin will use the source: tag of the archive if it exists (ex.: source:https://hitomi.la/XXXXX/XXXXX).",
    version: "0.3",
    icon: "data:image/png;base64,",
    oneshot_arg: "Hitomi Gallery URL (Will attach tags matching this exact gallery to your archive)",
  };
}


export async function execMetadata(hostArgs: Record<string, unknown>) {
  {
    const info = hostArgs as Record<string, any>;
    info.user_agent = perlCompat.userAgent();
    for (const c of (info.user_agent_cookies ?? []) as { name: string; value: string; domain: string; path: string }[]) {
      info.user_agent.cookie_jar.add(c);
    }
  }
  let match: RegExpMatchArray | null;
  // (shift) discarded positional arg — legacy Perl-OOP invocant/first @_ slot
  let lrr_info = hostArgs as Record<string, any>;
  //# Global info hash
  let logger = perlCompat.getLogger("Hitomi", "plugins");
  //    # Work your magic here - You can create subs below to organize the code better

  let galleryID = "";
  //    # Quick regex to get the hitomi gallery id from the provided url or source tag.

  logger.debug("Input param is " + lrr_info["oneshot_param"]);
  logger.debug("Regex is " + "/[.*-|.*\/]([0-9]+)\.html.*/");
  if ((match = lrr_info["oneshot_param"].match(/[.*-|.*\/]([0-9]+)\.html.*/))) {
    galleryID = match[1];
    logger.debug(`Skipping search and using gallery ${galleryID} from oneshot args`);
  } else if ((match = lrr_info["existing_tags"].match(/.*source:\s*hitomi\.la\/(?:manga|doujinshi|galleries|cg)\/.*-?([0-9]*)\.html.*/gi))) {
    galleryID = match[1];
    logger.debug(`Skipping search and using gallery ${galleryID} from source tag`);
  } else {
    //        #Get Gallery ID by hand if the user didn't specify a URL
  
    galleryID = get_gallery_id_from_title(lrr_info["file_path"]);
  }
  //    # Did we detect a Hitomi gallery?

  if (! galleryID) {
    let message = "No matching Hitomi Gallery Found!";
    logger.info(message);
    throw new Error(`${message}\n`);
  }
  logger.debug(`Detected Hitomi gallery id is ${galleryID}`);
  let hashdata = await get_tags_from_Hitomi(galleryID);
  logger.info("Sending the following tags to LRR: " + hashdata["tags"]);
  //    #Return a hash containing the new metadata - it will be integrated in LRR.

  return hashdata;
}

function get_gallery_id_from_title(...args: any[]) {
  let match: RegExpMatchArray | null;
  let file = args[0];
  let [title, filepath, suffix] = perlCompat.fileparse(file);
  let logger = perlCompat.getLogger("Hitomi", "plugins");
  logger.debug(`Attempting to parse id from title ${title}`);
  if ((match = title.match(/\{?(\d+)\}?/gm))) {
    logger.debug(`Got ${1} from file.`);
    return match[1];
  }
  return;
}

async function get_js_from_hitomi(...args: any[]) {
  let match: RegExpMatchArray | null;
  let [gID] = args.slice(0);
  let logger = perlCompat.getLogger("Hitomi", "plugins");
  let gJS = `https://ltn.gold-usergeneratedcontent.net/galleries/${gID}.js`;
  logger.debug(`Hitomi JS: ${gJS}`);
  let ua = perlCompat.userAgent();
  let res = (await ua.get(gJS)).result;
  logger.debug("Hitomi raw JS: " + res.body);
  if (res.is_error) {
    return;
  }
  let jsonstring = "{}";
  if ((match = res.body.match(/var.*galleryinfo.*= (.*)/gmi))) {
    jsonstring = match[1];
  }
  logger.debug(`Tentative new JSON: ${jsonstring}`);
  logger.debug("Beginning JSON decode");
  let json = JSON.parse(jsonstring);
  logger.debug("JSON decode successful");
  return json;
}

function get_tags_from_taglist(...args: any[]) {
  let [json] = args.slice(0);
  let logger = perlCompat.getLogger("Hitomi", "plugins");
  let tags = [];
  if ((json !== undefined && json !== null)["tags"]) {
    logger.debug("Extracting tags array");
    let tags_list = json["tags"];
    logger.debug("Cycling tags array");
    for (let tag of tags_list) {
      let name = tag["tag"];
      let male = tag["male"];
      let female = tag["female"];
      if (male) {
        name = `male:${name}`;
      }
      if (female) {
        name = `female:${name}`;
      }
      tags.push(name);
    }
  }
  if ((json !== undefined && json !== null)["parodys"]) {
    tags.push(extract_tags(logger, json, "parodys", "parody"));
  }
  if ((json !== undefined && json !== null)["artists"]) {
    tags.push(extract_tags(logger, json, "artists", "artist"));
  }
  if ((json !== undefined && json !== null)["groups"]) {
    tags.push(extract_tags(logger, json, "groups", "group"));
  }
  if ((json !== undefined && json !== null)["characters"]) {
    tags.push(extract_tags(logger, json, "characters", "character"));
  }
  logger.debug("Extracting type value");
  tags.push("type:" + json["type"]);
  logger.debug("Extracting language value");
  tags.push("language:" + json["language"]);
  return tags;
}

function extract_tags(...args: any[]) {
  let [logger, json, arrayname, namespace] = args.slice(0);
  let tags = [] as any[];
  logger.debug(`Extracting ${arrayname} array`);
  let list = json[arrayname];
  logger.debug(`Cycling ${arrayname} array`);
  for (let tag of list) {
    let name = tag[namespace];
    tags.push(`${namespace}:${name}`);
  }
  return tags;
}

function get_title_from_json(...args: any[]) {
  let [json] = args.slice(0);
  return json["title"];
}

async function get_tags_from_Hitomi(...args: any[]) {
  let [gID] = args.slice(0);
  let hashdata: Record<string, any> = { tags: "" };
  let logger = perlCompat.getLogger("Hitomi", "plugins");
  let json = await get_js_from_hitomi(gID);
  if (json) {
    logger.debug("Got fully formed JS from Hitomi");
    let tags = get_tags_from_taglist(json);
    if ((tags > 0)) { tags.push(`source:https://hitomi.la/galleries/${gID}.html`); }
    hashdata["tags"] = tags.join(', ');
    hashdata["title"] = get_title_from_json(json);
  }
  return hashdata;
}
