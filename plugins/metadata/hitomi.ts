// Converted from LANraragi/lib/LANraragi/Plugin/Metadata/Hitomi.pm via
// `lanrurugi-plugin-converter` (`cargo run -p lanrurugi-plugin-converter -- convert ...`),
// regenerated by `scripts/convert-plugins.sh` on 2026-07-19.
//
// Hand-fixed since regeneration:
//   - all three `/g`-flag warnings were false alarms for single-match-then-capture use (`.match()`
//     without `g` already returns `[fullMatch, group1, ...]` in JS) — the stray `g` flags (which
//     Perl needs for its own `while (/.../g)` iteration idiom, not used here) were just dropped.
//   - `extract_tags(...)` returns an array (Perl's `push(@tags, extract_tags(...))` flattens it
//     into the caller's list) — the converter's literal `tags.push(extract_tags(...))` instead
//     pushed one nested array as a single element; fixed to `tags.push(...extract_tags(...))`.
//   - `declared_permissions.net`: this plugin only ever calls `ltn.gold-usergeneratedcontent.net`
//     (Hitomi's own gallery-info JS host), verified against the one URL literal in
//     `get_js_from_hitomi`.

export function pluginInfo() {
  return {
    namespace: "hitomiplugin",
    type: "metadata" as const,
    parameters: [
    ],
    declared_permissions: { net: ["ltn.gold-usergeneratedcontent.net"], read: false, write: false },
    name: "Hitomi",
    author: "thelastfantasy",
    description: "Searches Hitomi.la for tags matching your archive.\n          <br>Supports reading the ID from files formatted as \"{Id} Title\" (curly brackets optional)\n\t\t  <br><i class='fa fa-exclamation-circle'></i> This plugin will use the source: tag of the archive if it exists (ex.: source:https://hitomi.la/XXXXX/XXXXX).",
    version: "0.1",
    icon: "data:image/png;base64,",
    oneshot_arg: "Hitomi Gallery URL (Will attach tags matching this exact gallery to your archive)",
  };
}


const GALLERY_ID_FROM_URL_PATTERN = /[.*-|.*\/]([0-9]+)\.html.*/;
const GALLERY_ID_FROM_SOURCE_TAG_PATTERN = /.*source:\s*hitomi\.la\/(?:manga|doujinshi|galleries|cg)\/.*-?([0-9]*)\.html.*/i;
const GALLERY_ID_FROM_TITLE_PATTERN = /\{?(\d+)\}?/;
const GALLERY_INFO_JS_ASSIGNMENT_PATTERN = /var.*galleryinfo.*= (.*)/im;

interface HitomiTagEntry {
  tag: string;
  male?: boolean;
  female?: boolean;
}

interface HitomiGalleryJson {
  tags?: HitomiTagEntry[];
  parodys?: { parody: string }[];
  artists?: { artist: string }[];
  groups?: { group: string }[];
  characters?: { character: string }[];
  type: string;
  language: string;
  title: string;
}

export async function execMetadata(hostArgs: Record<string, unknown>) {
  {
    const info = hostArgs as Record<string, any>;
    info.user_agent = legacyCompat.userAgent();
    for (const c of (info.user_agent_cookies ?? []) as { name: string; value: string; domain: string; path: string }[]) {
      info.user_agent.cookie_jar.add(c);
    }
  }
  interface ExecMetadataInfo extends Required<Pick<MetadataHostArgs, "arg" | "existing_tags" | "file_path">> {
    user_agent: LegacyUserAgent;
    user_agent_cookies?: LegacyCookie[];
  }
  const lrr_info = hostArgs as unknown as ExecMetadataInfo;
  const logger = legacyCompat.getLogger("Hitomi", "plugins");

  let galleryID: string | undefined = "";
  // Quick regex to get the hitomi gallery id from the provided url or source tag.
  logger.debug("Input param is " + lrr_info.arg);
  logger.debug("Regex is " + GALLERY_ID_FROM_URL_PATTERN);

  let match = (lrr_info.arg ?? "").match(GALLERY_ID_FROM_URL_PATTERN);
  if (match) {
    galleryID = match[1];
    logger.debug(`Skipping search and using gallery ${galleryID} from oneshot args`);
  } else if ((match = lrr_info.existing_tags.match(GALLERY_ID_FROM_SOURCE_TAG_PATTERN))) {
    galleryID = match[1];
    logger.debug(`Skipping search and using gallery ${galleryID} from source tag`);
  } else {
    // Get Gallery ID by hand if the user didn't specify a URL
    galleryID = get_gallery_id_from_title(lrr_info.file_path);
  }

  if (!galleryID) {
    const message = "No matching Hitomi Gallery Found!";
    logger.info(message);
    throw new PluginErrorException(message);
  }
  logger.debug(`Detected Hitomi gallery id is ${galleryID}`);
  const hashdata = await get_tags_from_Hitomi(galleryID);
  logger.info("Sending the following tags to LRR: " + hashdata.tags);
  return hashdata;
}

function get_gallery_id_from_title(file: string): string | undefined {
  const [title] = legacyCompat.fileparse(file);
  const logger = legacyCompat.getLogger("Hitomi", "plugins");
  logger.debug(`Attempting to parse id from title ${title}`);
  const match = title.match(GALLERY_ID_FROM_TITLE_PATTERN);
  if (match) {
    logger.debug(`Got ${match[1]} from file.`);
    return match[1];
  }
  return undefined;
}

async function get_js_from_hitomi(gID: string): Promise<HitomiGalleryJson | undefined> {
  const logger = legacyCompat.getLogger("Hitomi", "plugins");
  const gJS = `https://ltn.gold-usergeneratedcontent.net/galleries/${gID}.js`;
  logger.debug(`Hitomi JS: ${gJS}`);
  const ua = legacyCompat.userAgent();
  const res = (await ua.get(gJS)).result;
  logger.debug("Hitomi raw JS: " + res.body);
  if (res.is_error) {
    return undefined;
  }
  let jsonstring = "{}";
  const match = res.body.match(GALLERY_INFO_JS_ASSIGNMENT_PATTERN);
  if (match) {
    jsonstring = match[1];
  }
  logger.debug(`Tentative new JSON: ${jsonstring}`);
  logger.debug("Beginning JSON decode");
  const json = JSON.parse(jsonstring);
  logger.debug("JSON decode successful");
  return json;
}

function get_tags_from_taglist(json: HitomiGalleryJson): string[] {
  const logger = legacyCompat.getLogger("Hitomi", "plugins");
  const tags: string[] = [];

  if (json.tags) {
    logger.debug("Extracting tags array");
    logger.debug("Cycling tags array");
    for (const tag of json.tags) {
      let name = tag.tag;
      if (tag.male) name = `male:${name}`;
      if (tag.female) name = `female:${name}`;
      tags.push(name);
    }
  }
  if (json.parodys) tags.push(...extract_tags(logger, json.parodys, "parodys", "parody"));
  if (json.artists) tags.push(...extract_tags(logger, json.artists, "artists", "artist"));
  if (json.groups) tags.push(...extract_tags(logger, json.groups, "groups", "group"));
  if (json.characters) tags.push(...extract_tags(logger, json.characters, "characters", "character"));

  logger.debug("Extracting type value");
  tags.push("type:" + json.type);
  logger.debug("Extracting language value");
  tags.push("language:" + json.language);
  return tags;
}

function extract_tags(
  logger: LegacyLogger,
  list: Record<string, string>[],
  arrayname: string,
  namespace: string,
): string[] {
  const tags: string[] = [];
  logger.debug(`Extracting ${arrayname} array`);
  logger.debug(`Cycling ${arrayname} array`);
  for (const tag of list) {
    const name = tag[namespace];
    tags.push(`${namespace}:${name}`);
  }
  return tags;
}

interface HitomiHashData {
  tags: string;
  title?: string;
}

async function get_tags_from_Hitomi(gID: string): Promise<HitomiHashData> {
  const hashdata: HitomiHashData = { tags: "" };
  const logger = legacyCompat.getLogger("Hitomi", "plugins");
  const json = await get_js_from_hitomi(gID);
  if (json) {
    logger.debug("Got fully formed JS from Hitomi");
    const tags = get_tags_from_taglist(json);
    if (tags.length > 0) tags.push(`source:https://hitomi.la/galleries/${gID}.html`);
    hashdata.tags = tags.join(", ");
    hashdata.title = json.title;
  }
  return hashdata;
}
